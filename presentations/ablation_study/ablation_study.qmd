...
<img src="../../images/fm_usage/router_usage_2026-06-16-17:51.png" class="router-usage-fig" alt="Bar chart of per-FM router usage; Curia and MRI-CORE exceed the uniform 1/K baseline.">
...
Controlled comparison: Cross-FM-random-pair/Router subset SSL · LP on raw `mri-core` (unless noted) · pretrained on MRNet · eval on kneeMRI ACL
...
<table class="fm-count-table">
  ...
  <thead>
    <tr>
      ...
      <th colspan="2">Cross-FM random pair</th>
      <th colspan="2">Router subset SSL</th>
    </tr>
    <tr>
      <th>AUROC</th>
      <th>AUPRC</th>
  ...
  <tbody>
    <tr class="fm-domain-natural">
      ...
      <td>0.5962 ± 0.0450<sup>[a]</sup></td>
      <td>0.3964 ± 0.0433<sup>[a]</sup></td>
      <td class="metric-na">—<sup>[c]</sup></td>
      <td class="metric-na">—<sup>[c]</sup></td>
    </tr>
    ...
    <tr class="fm-domain-dino">
      ...
      <td>0.7022 ± 0.0080</td>
      <td>0.4443 ± 0.0149</td>
      <td><strong>0.7486 ± 0.0241</strong></td>
      <td><strong>0.5094 ± 0.0557</strong></td>
    </tr>
    ...
    <tr class="fm-domain-mixed">
      ...
      <td><u>0.7235 ± 0.0032</u><sup>[b]</sup></td>
      <td>0.4599 ± 0.0203<sup>[b]</sup></td>
      <td>0.5409 ± 0.0433<sup>[b]</sup></td>
      <td>0.3787 ± 0.0154<sup>[b]</sup></td>
    </tr>
...
::: {.table-footnote}
<sup>[a]</sup> MedImageInsight (1024-D) as the pooling target. MRI-CORE probing is unavailable because pretraining did not include a 768-D projection head. 

<sup>[b]</sup> Mixed w/o MRI-specialist set has no MRI-CORE in pretraining. <code>rad-dino</code> is used as the pooling target instead.

<sup>[c]</sup> Router subset SSL requires ≥3 FMs to form distinct contrastive pairs; with only 2 FMs the router falls back to cross-FM random pair.
:::
...
## Cross-FM has more flexibility in the choice of pooling target compared to Router subset SSL

In cross-FM SSL, `per_dim` EmbedMLP learns one head per embed dim. FM-agnostic in inference only holds if the feature dimension was seen in pretraining (e.g. MRI-CORE 768-D from RAD-DINO; MedImageInsight 1024-D from DINOv2). However, in router subset SSL, the FM choices in inference are restricted to the subset of FMs used in pretraining.
:::
...
# Plotly.js is loaded once in the document header by presentations/_slides.yml.
# Keeping the bundle out of cell output ensures this div remains inside its slide.
display(
    HTML(
        fig.to_html(
...
series = {
    "FM Router SSL mode": dict(
        x=[0, 1, 2],
        y=[0.7443, 0.7695, 0.7840],
        e=[0.0066, 0.0152, 0.0102],
...
display(
    HTML(
        fig.to_html(
            include_plotlyjs=False,  # loaded once in the document header
            full_html=False,
            config={"displayModeBar": False, "responsive": True},
            post_script=_post_script,
        )
...
- **FM-Router-SSL:** Expanding pretraining from MRNet to MRNet+fastMRI+KMAR50K improves AUROC by **+4.7 pp** (0.7226 → **0.7695**); adding **OAI** yields a further **+1.5 pp** to **0.7840 ± 0.0102**, the strongest kneeMRI ACL result in this comparison.
...
## Idea for paper targets {.paper-targets-slide}

1. **Motivation**
   - No universally best 2D pretrained model across tasks; Anna Karenina principle
   - Driven by COBRA, feature-space pretraining remains underexplored

2. **Main contributions**
   - Propose MedSliM; evaluate across tasks/datasets; competitive vs strong baselines

3. **Future work**
   - Expand beyond limited 3D datasets; extend from classification toward segmentation/detection

::: {.callout-warning .fm-agnostic-callout .paper-comparison-callout}
## Benchmark framing: compare apples with apples

Task-supervised models optimize on labels, so they are not a fully matched fairness claim (Original DINO paper likewise reports competitive transfer, not universal superiority). Prefer SSL comparison, **[3DINO](https://arxiv.org/abs/2501.11755)** (selling point: comparable accuracy at lower compute) and optionally **[VoCo](https://arxiv.org/abs/2507.08254)**. Keep supervised models as reference.
:::
